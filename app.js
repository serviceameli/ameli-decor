...
function render(content) {
  ...
  const longerTerms=new Set(['Изменения и дополнения','Визуализация на площадке']);
  const termColumns=[
    {side:'right',items:content.terms.filter(term=>longerTerms.has(term.title))},
    {side:'left',items:content.terms.filter(term=>!longerTerms.has(term.title))}
  ];
  let termNumber=0;
  $('#terms-list').innerHTML=termColumns.map(column=>`<div class="terms-column terms-column--${column.side}">${column.items.map(term=>`<article class="term"><span>${String(++termNumber).padStart(2,'0')}</span><div><h3>${escape(term.title)}</h3><p>${escape(term.text)}${term.title==='Изменения и дополнения'?' <a class="terms-catalog-link" href="https://catalog.ameli-rental.ru/" target="_blank" rel="noopener noreferrer">Открыть каталог ↗</a>':''}</p></div></article>`).join('')}</div>`).join('');
